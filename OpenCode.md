...
| `compositingGroup().clipShape(Rectangle())` 在 ZStack 上 | ✅ | 裁切在动画作用域内，帧同步 |
...
点击非相邻 tab 时（如从 A 直接点 C），触发的是 `.spring()` 滑动动画而不是用户设置的 Transition Style（fade/slide）。应统一使用用户在 Interface → Transition Style 中选择的动画，不管点击的是否相邻 tab。

### Dashboard 日历排版 + 事件编辑
Dashboard 的日历部分排版有问题——日历太靠近 dashboard 区域左侧。后续需一并处理：
- 日历排版间距
- 事件双击编辑/右键删除功能（已实现过又被移除，可参考 git history 恢复）
- Sheet 弹窗灰色背景（macOS `.sheet()` 默认系统标题栏背景色，尝试过 `ignoresSafeArea()` 和 in-content overlay 均不理想）
...
Overview 界面最右侧会渲染当前播放的封面（`NowPlayingArtworkBackground` 或类似视图），之前修过一次但又复现了。需要排查哪次 commit 引入了回归，以及 `MusicPlayerView` 或 `OverviewView` 中是否有泄漏的 `.background` / overlay 把封面渲染到了不该出现的位置。
