...
**已修复（第二次）：**
- **根本原因**：系统电源状态可能波动（`false → true → false → true`），如果波动间隔超过防抖窗口，就会发送两次 `.charger` 事件，导致 `NotchEngine` 先 hide 再 show，造成闪烁
- **修复方案**：
  1. 防抖时间从 100ms 延长到 200ms
  2. 添加事件抑制机制：2 秒内相同类型的电源事件只发送一次（`lastSentEvent` + `lastSentTime` + `eventSuppressionInterval: 2.0`）
  3. 使用 `pendingEvent` 变量确保防抖期间只保留最后一个事件
- **文件**：`OpenNotch/Features/Battery/PowerViewModel.swift`
...
- 可能与 `CalendarStore` 的权限判断逻辑（`isGranted` 只检查 `.fullAccess`，而系统授权的是 `.writeOnly`）或轮询刷新机制未触发有关
- **未见用户明确要求修复，待确认**
